...
import { startDeliveryForDriver } from "@/lib/data/deliveries";
...
export async function PATCH(_request: Request, context: RouteContext) {
  ...
  try {
    const delivery = await startDeliveryForDriver(id, user.id);
    ...
    if (e instanceof Error && e.message === "INVALID_STATE") {
      return jsonBadRequest("Delivery cannot be started in the current state");
    }
    throw e;
  }
}
